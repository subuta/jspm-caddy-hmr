...
const reload = (path) => {
  ...
  if (isBundled) {
    ...
    if (_.last(parentModules)) {waitsForImport.push(_.first(parentModules));}
...
export const traceToRootModule = (url, result = []) => {
  ...
  const parentModuleUrls = _.reduce(tree, (results, dependencies, key) => {
    if (_.some(dependencies, dependency => dependency === url)) {
      results.push(key);
    }
    return results;
  }, []);

  if (_.isEmpty(parentModuleUrls)) {
    ...
    const moduleDependencies = _.map(parentModuleUrls, parentModuleUrl => {
      return traceToRootModule(parentModuleUrl, [...result, parentModuleUrl]);
    });
    // flatten array and make each dependencies unique.
    return _.uniq(_.flattenDeep(moduleDependencies));
  }
};
