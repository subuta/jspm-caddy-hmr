...
const reload = (path) => {
  ...
  if (isBundled) {
    ...
    // const rootModule = traceToRootModule(url);
    // deleteWithHook(rootModule);
    // waitsForReload.push(rootModule);
  ...
  if(isBundleFile) {
    ...
    promise = Promise.all(promises).then((imported) => {
      // reload module.
      _.each(waitsForReload, (moduleUrl) => {
        importWithHook(moduleUrl);
        // const module = System.get(moduleUrl);
        // module._reload && module._reload();
      });
